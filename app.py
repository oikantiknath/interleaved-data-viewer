...
# LOCAL_BASE = pathlib.Path("/Users/oikantik/wiki-data-samples")

import pathlib
APP_DIR = pathlib.Path(__file__).parent
BASE = APP_DIR / "data" / "wiki-data-samples" 
ORIG_PREFIX = pathlib.Path("/data/oikantik_sarvam_ai/wiki-data")
LOCAL_BASE = BASE
...
def list_langs(domain: str) -> List[str]:
    d = BASE/domain
    langs = set()
    for sub in ("markdown","html"):
        p = d/sub
        if p.exists():
            for c in p.iterdir():
                if c.is_dir(): langs.add(c.name)
    return sorted(langs)

@st.cache_data
def list_articles(domain: str, lang: str) -> List[str]:
    d = BASE/domain
    html_stems = {p.stem for p in (d/"html"/lang).glob("*.html")} | {p.stem for p in (d/"html"/lang).glob("*.htm")}
...
def load_paths(domain: str, lang: str, stem: str) -> Dict[str, pathlib.Path]:
    ...
    if not html.exists(): html = (d/"html"/lang/f"{stem}.htm")
...
def img_to_data_uri(p: pathlib.Path) -> str | None:
    ...
    mime = "image/jpeg" if suffix in (".jpg",".jpeg") else "image/png" if suffix == ".png" else "image/gif" if suffix == ".gif" else "application/octet-stream"
...
if not domains:
    st.warning("No domains found under the local dataset path."); st.stop()
...
if not articles:
    st.warning("No overlapping HTML+Markdown articles for this domain/lang."); st.stop()
with c2:
...
with left:
    ...
    if rec and rec.get("page_url"): page_url = rec["page_url"]
    if not page_url: page_url = canonical_url_from_html(paths["html"])
    # if page_url:
    #     try:
    #         components.iframe(page_url, height=900, scrolling=True)  # scrollable
    #     except Exception:
    #         components.html(f'<iframe src="{page_url}" style="width:100%; height:900px; border:none; overflow:auto" allowfullscreen></iframe>', height=900)
    if page_url:
        try:
...
with right:
    ...
    if not rec: rec = load_json_record(paths["json"])
    if rec:
        ...
        components.html(
            f"""<div style="height:900px; overflow:auto; border:1px solid #ddd; padding:8px; white-space:pre-wrap; font-family:monospace">{md_text.replace("&","&amp;").replace("<","&lt;").replace(">","&gt;")}</div>""",
            height=900,
            scrolling=False
        )
